...
namespace FIMSpace.BonesStimulation
{
    public partial class BonesStimulator
    {
        ...
        private void Vibrate_Calculation(Bone target)
        {
            ...
            if (VibrateAxis.z != 0)
            {
                vibrationFactor.y = _vib_cosinuses[target.randomInts[4]] * _vib_Range * target.randomFloats[4];
                vibrationFactor.y += _vib_sinuses[target.randomInts[5]] * _vib_Range * target.randomFloats[5];
                vibrationFactor.y *= VibrateAxis.y;
            }
            ...
            if (VibrateRotation != 0f)
            {
                ...
                if (rotBlend > 0f) target.transform.localRotation *= Quaternion.Euler(vibrationFactor * VibrateRotation * rotBlend);
            }

...
namespace HutongGames.PlayMaker.Actions
{
	...
	public class TakeCameraScreenshot : FsmStateAction
	{
		...
		public void getPicture()
		{
			...
			else if (!useMypictures.Value && useDefaultFolder.Value == true) screenshotPath = Application.dataPath;



			if (!useJpeg.Value) screenshotFullPath = filename.Value + ".png";
			else screenshotFullPath = filename.Value + ".jpg";
			...
			if (autoNumber.Value)
			{
				while (System.IO.File.Exists(screenshotFullPath)) 
				{
					screenshotCount++;
					if (!useJpeg.Value)
						screenshotFullPath = filename.Value + screenshotCount + ".png";
					else screenshotFullPath = filename.Value + screenshotCount + ".jpg";
				} 
			}
